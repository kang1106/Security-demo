...
public:
    ...
    crypto_service() :
        ...
        stop_(false),
        stop_thread_(std::bind(&crypto_service::stop, this))
    {
    }
    ...
    bool init()
    {
        ...
        app_->register_message_handler(service_id, service_instance_id,
                service_method_id,
                std::bind(&crypto_service::on_message_cbk, this,
                        std::placeholders::_1));

        // register a state handler to get called back after registration at the
        // runtime was successful
        app_->register_state_handler(
                std::bind(&crypto_service::on_state_cbk, this,
                        std::placeholders::_1));
        return true;
    }
